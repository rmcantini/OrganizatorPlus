...
try:
    ...
    for file in names:
        if "site" in file:
            new_path = os.path.isfile(shutil.move(f"{path_main}/{file}", NEWFOLDER_1))

        elif "voxus" in file:
            new_path = os.path.isfile(shutil.move(f"{path_main}/{file}", NEWFOLDER_2))

        elif "GDN" in file:
            new_path = os.path.isfile(shutil.move(f"{path_main}/{file}", NEWFOLDER_3))

        elif "vtex" in file:
            new_path = os.path.isfile(shutil.move(f"{path_main}/{file}", NEWFOLDER_5))

        elif "dark" in file:
            new_path = os.path.isfile(shutil.move(f"{path_main}/{file}", NEWFOLDER_4))
